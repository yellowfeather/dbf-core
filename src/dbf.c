 ...
 */
static int
setShowDel (FILE *output, P_DBF *p_dbf,
    const char *filename, const char *level)
{
    if (level[1] != '\0' || level[0] < '0' || level[0] > '1') {
        fprintf(stderr, _("Invalid showdel value ``%s''. Must be either 0 or 1"), level);
        fprintf(stderr, "\n");
        return 1;
    }
    setKeepDel(output, p_dbf, filename, level);
    show_deleted = level[0] - '0';
    return 0;
}
/* }}} */
...
struct options {
    ...
    {
        "--showdel", setShowDel, NULL, NULL, ARG_OPTION, ARG_CLASS_SET,
        "0 or 1 -- whether to output deleted records and deleted flag in CSV",
        "0"
    },
    {
